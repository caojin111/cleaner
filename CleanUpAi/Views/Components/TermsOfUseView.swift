...
struct TermsOfUseView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .onAppear {
                Logger.ui.debug("TermsOfUseView: 初始化使用条款视图")
                loadTermsOfUse()
            }
        }
    }
    ...
    private func loadTermsOfUse() {
        ...
        if let bundleURL = Bundle.main.url(forResource: "TermsOfUse", withExtension: "html") {
            do {
                ...
                Logger.ui.debug("从Bundle成功读取使用条款HTML文件，长度: \(content.count)")
                htmlContent = content
                isLoading = false
                return
            } catch {
                Logger.ui.error("从Bundle读取使用条款HTML文件失败: \(error.localizedDescription)")
            }
        }
        ...
        do {
            ...
            Logger.ui.debug("从文件系统成功读取使用条款HTML文件，长度: \(content.count)")
            htmlContent = content
            isLoading = false
            return
        } catch {
            Logger.ui.error("从文件系统读取使用条款HTML文件失败: \(error.localizedDescription)")
        }
        
        // 方案3：使用内联HTML作为备用
        Logger.ui.debug("使用内联HTML作为使用条款备用方案")
